        ...
        for row in rows:
            ...
            if isinstance(receipt, dict) and receipt.get("tradeId") == row[0]:
                updated = await db.execute(
                    "UPDATE CryptoTrade SET status='COMMITTED',receiptJson=?,settledAt=?,"
...
async def claim_crypto_news_outbox(db_path, *, lease_owner, limit=100, lease_seconds=120):
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        await db.execute(
            ...
            "lastErrorCode='expired_lease' WHERE status='CLAIMED' AND leaseExpiresAt<?", (now,),
        )
        async with db.execute(
            ...
            "WHERE o.status IN ('PENDING','FAILED') ORDER BY o.createdAt LIMIT ?", (int(limit),),
        ) as cursor:
            rows = await cursor.fetchall()
        claimed = []
        for row in rows:
            updated = await db.execute(
                "UPDATE CryptoNewsOutbox SET status='CLAIMED',leaseOwner=?,leaseExpiresAt=?,"
...
async def finalize_crypto_news_outbox(db_path, *, outbox_id, lease_owner, sent,
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        updated = await db.execute(
            "UPDATE CryptoNewsOutbox SET status=?,messageId=?,lastErrorCode=?,sentAt=?,"
