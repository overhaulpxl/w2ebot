...
async def list_pets(db_path, guild_id, user_id):
    async with _pool.acquire() as db:
        ...
        pet = await db.fetchrow(
            ...
            "FROM RpgPetInstance WHERE guildId=$1 AND ownerId=$2 ORDER BY rarity,createdAt",
            (str(guild_id), str(user_id)),
        ) as cursor:
    ...
    async with _pool.acquire() as db:
        ...
        try:
            async with db.execute(
                "SELECT petId,status,catalogVersion FROM RpgPetInstance WHERE petInstanceId=$1 AND guildId=$2 AND ownerId=$3",
            ...
            await db.execute(
                "UPDATE RpgProfile SET activePetInstanceId=$1,version=version+1,updatedAt=$2 WHERE guildId=$3 AND userId=$4",
...
async def grant_pet_xp_in_transaction(db, *, guild_id, user_id, pet_instance_id, amount, now):
    if not pet_instance_id or amount <= 0:
        return False
    pet = await db.fetchrow(
        "SELECT level,xp,status FROM RpgPetInstance WHERE petInstanceId=$1 AND guildId=$2 AND ownerId=$3",
        (str(pet_instance_id), str(guild_id), str(user_id)),
    )
    ...
    cursor = await db.execute(
        "UPDATE RpgPetInstance SET level=$1,xp=$2,updatedAt=$3 WHERE petInstanceId=$4 AND guildId=$5 AND ownerId=$6 AND status='OWNED'",
