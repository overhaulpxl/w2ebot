...
async def _existing_operation(db, request_id, identity):
    async with db.execute(
        ...
        "receiptJson,errorCode FROM DashboardControlledOperation WHERE requestId=?", (request_id,),
    ) as cursor:
...
async def change_permission(db, *, action, guild_id, actor_id, target_user_id, permission_class,
    ...
    await db.execute(
        ...
        "expectedVersion,status,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?,'PENDING',?)",
        (operation_id, request_id, *identity[:7], identity[7], iso(moment)),
    )
    await db.execute(
        ...
        (str(guild_id), str(target_user_id), iso(moment), iso(moment)),
    )
    if action == "GRANT":
        ...
        if existing:
            ...
            await db.execute(
                ...
                "VALUES (?,?,?,?,'ACTIVE',?,?)",
                (assignment_id, str(guild_id), str(target_user_id), permission_class, str(actor_id), iso(moment)),
            )
            resulting_version = 0
    else:
        existing = await _active_assignment(db, guild_id, target_user_id, permission_class)
        ...
        cursor = await db.execute(
            "UPDATE DashboardOperatorPermission SET status='REVOKED',revokedById=?,revokedAt=?,version=version+1 "
            "WHERE assignmentId=? AND status='ACTIVE' AND version=?",
            (str(actor_id), iso(moment), assignment_id, current_version),
        )
        if cursor.rowcount != 1:
            raise DashboardSecurityError("version_conflict", 409)
    await db.execute(
        "UPDATE DashboardSession SET status='REVOKED',revokedAt=?,revokeReasonCode='PERMISSION_CHANGED',version=version+1 "
    ...
    await db.execute(
        "UPDATE DashboardControlledOperation SET status='COMMITTED',resultingVersion=?,receiptJson=?,receiptHash=?,settledAt=? "
        "WHERE operationId=? AND status='PENDING'",
        (resulting_version, receipt_json, receipt_digest, iso(moment), operation_id),
    )
    await db.execute(
    ...
    await db.execute(
        ...
        "resultingVersion,receiptHash,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        (str(uuid.uuid4()), str(guild_id), str(target_user_id), permission_class, action, str(actor_id),
...
async def _active_assignment(db, guild_id, user_id, permission_class):
    async with db.execute(
        "SELECT assignmentId,version FROM DashboardOperatorPermission WHERE guildId=? AND userId=? "
...
async def revoke_dashboard_session(db, *, guild_id, actor_id, target_session_id, request_id,
    ...
    if not await has_permission(db, guild_id, actor_id, "DASHBOARD_SECURITY_ADMIN"):
        raise DashboardSecurityError("forbidden", 403)
    async with db.execute(
        "SELECT guildId FROM DashboardSession WHERE sessionId=?", (str(target_session_id),),
    ) as cursor:
