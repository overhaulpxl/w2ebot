"""CLI migrasi staging Mining Phase 7."""
...
from economy.phase7_migrations import (
    apply_phase7_staging, phase7_dry_run, reconcile_phase7_staging,
    restore_phase7_staging, verify_phase7_staging,
)
from runtime_config import PRODUCTION_DATABASE_PATH


def main(argv=None):
    parser = argparse.ArgumentParser(description="Migrasi eksplisit Phase 7 Mining")
    ...
    if args.mode == "dry-run":
        result = phase7_dry_run(args.database)
        ...
        result = verify_phase7_staging(args.database)
    elif args.mode == "reconcile":
        result = reconcile_phase7_staging(args.database)
    else:
        if not args.backup:
            parser.error("--backup wajib untuk restore")
        result = restore_phase7_staging(
