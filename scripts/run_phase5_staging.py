"""Launcher fail-closed Casino Phase 5; hanya membaca .env.staging."""
...
def read_staging_env(path=STAGING_ENV):
    ...
    required = ("STAGING_MODE", "STAGING_GUILD_ID", "DATABASE_PATH", "DISCORD_TOKEN",
                "ECONOMY_V1_ENABLED", "ECONOMY_PHASE2_ENABLED", "ECONOMY_PHASE5_ENABLED")
    missing = [key for key in required if not values.get(key)]
    if missing:
        raise RuntimeError("Konfigurasi staging belum lengkap: " + ", ".join(missing))
    if values["STAGING_MODE"].lower() != "true" or any(values[key].lower() != "true" for key in required[-3:]):
        raise RuntimeError("Staging Phase 5 memerlukan STAGING_MODE, V1, Phase 2, dan Phase 5 true.")
...
def main():
    ...
    from economy.phase5_migrations import verify_phase5_staging
    config = validate_startup_configuration()
    verification = verify_phase5_staging(config.database_path)
    if not config.casino_flags_enabled or config.uses_production_database or not verification["schemaCapable"]:
        raise RuntimeError("Launcher menolak konfigurasi atau schema Casino staging.")
